...
![](/images/JUnit5-01.png)

| jar 包                  | 说明                                                         |
| :---------------------- | :----------------------------------------------------------- |
| JUnit Platform          | 提供平台功能的模块。通过 JUnit Platform ，其他的自动化测试引擎（例如 TestNG）都可以接入 JUnit 实现对接和执行。 |
| JUnit Jupiter           | JUnit5 的核心，可以看作是承载 JUnit4 升级演进。包含了很多丰富的新特性。 |
| JUnit Vintage（非必须） | 这个模块是对 JUnit3、JUnit4 版本兼容的测试引擎，使旧版本 junit 的自动化测试脚本也可以运行在 JUnit5 平台下。 如果你的测试代码并非基于 JUnit3、JUnit4 编写的，那么自然就不需要这个包。 |

![](/images/JUnit5-02.png)

使用 JUnit 5 进行测试需要引入的包有：

![](/images/JUnit5-05.png)
...
1. 在 eclipse 中创建 1 个 source folder 命名为 test（使用 Maven 后已要求必须创建）
...
Junit 5 引入了 **@DisplayName** 注解，通过它可以给指定测试报告中的名字，而不是显示类和方法的名字。
