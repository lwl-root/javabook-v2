...
这个时候可能有人会问：为什么不直接使用 Set 或者 LinkedHashSet 呢？这样不就没有重复数据的问题了嘛？

不得不说，很机智，一眼就看到了问题的本质。
...
本文介绍了 6 种集合去重的方法，其中实现最简洁，且去重之后的顺序能和原集合保持一致的实现方法，只有两种：LinkedHashSet 去重和 Stream 去重，而后一种去重方法无需借助新集合，是我们优先考虑的去重方法。
