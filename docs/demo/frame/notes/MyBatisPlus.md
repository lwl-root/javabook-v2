...
官方网站：[MyBatis-Plus](https://baomidou.com/)
## 1、快速入门
### 1.2、导入依赖
```xml
<!-- pom.xml -->
...
```
### 1.6、在SpringBoot启动类上配置mapper接口的扫描路径
```java
...
运行测试类,可以看到，针对单表的基本CRUD操作，只需要创建好实体类，并创建一个继承自BaseMapper的接口即可，可谓非常简洁。并且，我们注意到，User类中的managerId，createTime属性，自动和数据库表中的manager_id，create_time对应了起来，这是因为mp自动做了数据库下划线命名，到Java类的驼峰命名之间的转化。
