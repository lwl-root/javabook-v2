...
在编程世界中，我们日常生活中所说的『**秒**』使用的是 **second** ；而『**相较于原点的秒**』使用的是 **epoch second** 。**epoch** 是时代、纪元等含义。
...
因特网编号管理局（IANA）维护着一份全球所有已知时区的数据库（[http://www.iana.org/time-zonesopen in new window](http://www.iana.org/time-zones)），每年会更新几次（这些更新主要处理夏令时规则的改变）。Java 就是使用了 IANA 的数据库。
